...
class Feature2:
    """
    Find which types of issues the bot accounts (e.g. sourcery-ai) tend to engage with the most.
    - Graph output
    - Each bar is a bot user (from text-files/event_authors_bots.txt)
    - Y axis is the number of events authored by each bot (all event types)
    """
    ...
    def run(self):
        ### LOAD BOT USERNAMES ###
        with open("text-files/event_authors_bots.txt", "r", encoding="utf-8") as f:
